...
public:
    ...
    static QVector<QCPGraphData> time_data_aggregator(const std::vector<Drink>& all_drinks, double std_drink_size);
    static bool compare_by_date(const Drink &a, const Drink &b);
    static int parse_date(std::string &date);
    static QCustomPlot * plot_abvs(const QVector<QCPGraphData>& time_data, Options options, QDialog *parent);
